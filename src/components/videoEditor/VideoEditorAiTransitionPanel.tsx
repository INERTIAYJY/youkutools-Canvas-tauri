      ...
      {open && (
        <div className="video-editor-ai-transition-body">
          <p className="video-editor-ai-transition-hint">
            取前一段的尾帧作首帧、本段的首帧作尾帧，按提示词生成一段过渡视频并插入两段之间。
          </p>
          ...
          <label className="video-editor-field-stack">
            ...
            <textarea
              ...
              value={prompt}
              maxLength={500}
              placeholder="例如：镜头快速推近穿过火光，自然过渡到下一场景"
          ...
          <label className="video-editor-inspect-slider">
            ...
            >
              {models.length === 0 && <option value="">暂无可用视频模型</option>}
              {models.map((option) => (
          ...
          {!canGenerate && (
            <p className="video-editor-ai-transition-note">
              请先在主轨上选中第二段及之后的片段——转场要插在它与前一段之间。
            </p>
          )}
          ...
          {models.length === 0 && (
            <p className="video-editor-ai-transition-note">
              没有收到可用模型：请确认主窗口仍开着该视频节点所在的项目，并已在「设置 → API Key」中配置视频模型。
            </p>
          )}
        </div>
      )}
