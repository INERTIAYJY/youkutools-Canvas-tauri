...
function VideoEditorTextPanel({
  ...
  const ALIGN_OPTIONS: { value: VideoEditorTextAlign; icon: string; label: string }[] = [
    { value: 'left', icon: 'lucide:align-left', label: t('左对齐') },
    { value: 'center', icon: 'lucide:align-center', label: t('居中') },
    { value: 'right', icon: 'lucide:align-right', label: t('右对齐') },
  ];
  ...
  const loadLocalFonts = async () => {
    ...
    try {
      ...
      setFontOpen(true);
      setFontMessage(t('已读取 {count} 个本机字体', { count: Math.max(0, options.length - getSystemFontOptions().length) }));
    } catch (error) {
      setFontMessage(error instanceof Error ? error.message : t('读取本机字体失败'));
    } finally {
    ...
    <div className="video-editor-layer-panel">
      ...
      {isText ? (
        <div className="video-editor-layer-controls">
              ...
              <div className="video-editor-font-select-wrap">
                ...
                >
                  <span style={{ fontFamily: style.fontFamily }}>
                    {fontOptions.find((option) => option.value === style.fontFamily)?.label ?? t('当前字体')}
                  </span>
                ...
                {fontOpen && (
                  <div className="video-editor-font-menu">
                    ...
                    <div className="video-editor-font-options" role="listbox" aria-label={t('字体')}>
                      ...
                      {visibleFontOptions.length === 0 && (
                        <span className="video-editor-font-no-result">{t('没有匹配的字体')}</span>
                      )}
                    </div>
                  </div>
                )}
          ...
          <div className="video-editor-field-row">
            ...
            <div className="video-editor-segmented">
              {([400, 600, 700] as const).map((weight) => (
                ...
                >
                  {weight === 400 ? t('常规') : weight === 600 ? t('中等') : t('粗体')}
                </button>
              ))}
            </div>
          </div>
          ...
          <p className="video-editor-layer-hint">{t('可直接在预览画面拖动、缩放；显示时长在时间轴调整')}</p>
        </div>
      ) : (
        <div className="video-editor-layer-empty">
          <Icon icon="lucide:mouse-pointer-2" width={20} height={20} />
          <span>{t('添加或选中一段文字后编辑样式')}</span>
        </div>
      )}
    </div>
  );
