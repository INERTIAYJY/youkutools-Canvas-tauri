...
interface VideoEditorTextStickerPanelProps {
  mode: 'text' | 'sticker';
...
function VideoEditorTextStickerPanel({
  mode,
  selectedClip,
  selectedIsOverlay,
  projectImages,
  ...
  onBeginInteraction,
  onEndInteraction,
}: VideoEditorTextStickerPanelProps) {
  ...
  const selectedSticker = selectedIsOverlay && selectedClip?.kind === 'image';
  return (
    <div className="video-editor-layer-panel">
      <button
        type="button"
      ...
      >
        <Icon icon={uploading ? 'lucide:loader-circle' : 'lucide:upload'} width={16} height={16} />
        {uploading ? '正在导入…' : '上传本地贴图'}
      </button>

      <div className="video-editor-layer-section-head">
        <span>项目图片节点</span>
        <em>{projectImages.length}</em>
      </div>
      {projectImages.length > 0 ? (
        ...
        <div className="video-editor-layer-empty compact">
          ...
          <span>当前项目还没有可用的图片节点</span>
        </div>
      )}

      {selectedSticker && (
        <p className="video-editor-layer-hint selected">
          已选中「{selectedClip.fileName}」，可在预览画面调整位置和大小。
        </p>
      )}
    </div>
  );
...
export default memo(VideoEditorTextStickerPanel);
